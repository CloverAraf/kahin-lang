...
“Kahin” kodu AST’ye çevirir, optimize eder ve tekrar Python AST yapısına dönüştürür
Abstract Syntax trii kodun ağaç yapısı şeklinde temsil edilmesidir
...
Parse aşamasında syntax kontrolü yapılır
Optimizasyon imkânı sağlar   constant folding, dead code elimination gibi
Bytecode üretimine temel oluşturur
Hata mesajlarında satır ve konum bilgisi sunar

## performans

Python’un `ast` modülünü kullanır C ile implement edilmiştir
Oldukça hızlıdır: yaklaşık 1000 satır için 0.002s seviyesindedir
...
class KahinASTTransformer(ast.NodeTransformer):
    ...
    CPython'un ast.NodeTransformeri gibi çalışır
    Her node'u ziyaret edip dönüştürür

   sebebi:
    Lexer tokenları çeviriyor ama bu yeterli değil
    o yuzden AST seviyesinde optimizasyon lazım
    Constant Folding
    ...
    fuunction Inlining (gelecek)
    Variable Renaming Optimization (gelecek)
    """
    ...
    def visit_Name(self, node: ast.Name) -> ast.Name:
        """
        Name nodeslarını ziyaret et

        Türkçe değişken isimlerini pythona çevirebiliriz
        su an sadece keyword'ler ceviriliyor
        """
        # su an bir şey yapmıyoruz ama gelecekte
        #turkce değişken isimleri => snake_case
        # Reserved keyword kontrolü
        return node

    def visit_Constant(self, node: ast.Constant) -> ast.Constant:
        """
        Constant nodeslarini optimize et
    ...
    def visit_BinOp(self, node: ast.BinOp) -> Any:
        ...
        Örnek:
            ...
            x * 0  →  0
        """
        # Önce child nodesları ziyaret et
    ...
    def visit_Compare(self, node: ast.Compare) -> Any:
        ...
        Sadece tum operandlar sabit ve tek operatorlı zincirde foldlanır
        """
        self.generic_visit(node)
...
class KahinParser:
    ...
    def parse_and_compile(self, kaynak_kod: str, dosya_adi='<kahin>', optimize=2) -> Any:
        ...
        # Compile CPython'un bytecode compileri
