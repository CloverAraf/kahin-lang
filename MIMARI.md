...
Bu dosya Kahin langın iç işleyişini ve özellikle Rust hızlandırıcının pipelinein
...
1. **Önden-işleme.** Kaynaktaki Kahin'e özel şeyler saf Python'a çevriliyor:
...
5. **Parse ve optimize.** `ast.parse` ile ağaç çıkarılıyor, sonra
...
6. **Compile ve exec.** Ağaç bytecode'a derleniyor ve çalıştırılıyor. Bu
   noktadan sonra olan biten tamamen normal Python.

Yani Kahin'in kendine ait kısmı 1-3 arası. 4'ten sonrası CPython'un işi oluyor.

## Rust hızlandırıcı tam olarak ne yapıyor

En çok karıştırılan yer burasıdır, o yüzden ayrıca açayım.
...
- Binary sadece kendi platformuna (Linux x86-64) taşınabilir. Windows için
  Windows'ta derlemek gerekiyor - iyi kuuanimlar ↖️
